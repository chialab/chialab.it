 ...
 * @property \BEdita\Core\Model\Table\ObjectsTable $Lessons
 ...
 * @phpstan-type DocumentData array{status: 'on'|'draft'|'off', original_id: int, original_uname: string, title: string, description: string, body: string, publish_start: string|null, publish_end: string|null, extra: string|null, created: string|null, modified: string|null}
 * @phpstan-type LessonData array{status: 'on'|'draft'|'off', original_id: int, original_uname: string, title: string, description: string, body: string, extra: string|null, created: string|null, modified: string|null}
 * @phpstan-type AuthorData array{status: 'on'|'draft'|'off', original_id: int|null, original_uname: string, title: string, name: string|null, surname: string|null, email: string|null, created: string|null, extra: string|null, modified: string|null}
 * @phpstan-type MediaData array{status: 'on'|'draft'|'off', original_id: int, original_uname: string, name: string, title: string|null, description: string|null, provider: string|null, provider_uid: string|null, provider_url: string|null, provider_thumbnail: string|null, provider_extra: string|null, extra: string|null, created: string|null, modified: string|null}
 * @phpstan-type Be3Folder array{content_order: string, original_id: int, original_uname: string, status: 'on'|'draft'|'off', title: string, description: string, created: string, modified: string}
 * @phpstan-type Be3Content array{type_name: string, original_id: int, original_uname: string, status: 'on'|'draft'|'off', title: string, description: string, body: string, relation_params: string|null, extra: string|null, name: string|null, surname: string|null, email: string|null, publish_start: string|null, publish_end: string|null, url: string|null, created: string, modified: string}
...
trait ImportBe3Trait
{
     ...
     * @param DocumentData $data Data of the document to be imported
    ...
    protected function findOrCreateDocument(string $uname, array $data, array $contain = [], string|null $unamePrefix = null): ObjectEntity|false
    {
        ...
        $document = $this->Documents->newEntity(compact('uname'));
        $document->created_by = UsersTable::ADMIN_USER;
        $document->modified_by = UsersTable::ADMIN_USER;
        $document->created = $data['created'] ?? FrozenTime::now(); // @phpstan-ignore-line
        $document->modified = $data['modified'] ?? FrozenTime::now(); // @phpstan-ignore-line
        if (!empty($data['extra']) && is_string($data['extra'])) {
            $data['extra'] = json_decode($data['extra'], true);
        }

        $document = $this->Documents->patchEntity($document, array_merge_recursive($data, [
            'extra' => [
                'imported' => [
                    'id' => $data['original_id'],
    ...
    /**
     * Find the imported lesson or create a new one.
     *
     * @param string $uname Uname of the lesson to be imported
     * @param LessonData $data Data of the lesson to be imported
     * @param array<string> $contain Relations to load
     * @return \BEdita\Core\Model\Entity\ObjectEntity|false `false` if an object with the same uname already exists
     */
    protected function findOrCreateLesson(string $uname, array $data, array $contain = ['ComposedBy']): ObjectEntity|false
    {
        /** @var \BEdita\Core\Model\Entity\ObjectEntity|null $lesson */
        $lesson = $this->Lessons->find()
            ->where(compact('uname'))
            ->contain($contain)
            ->first();
        if ($lesson !== null) {
            if (Hash::get((array)$lesson->extra, 'imported.uname') === $data['original_uname'] && Hash::get((array)$lesson->extra, 'imported.id') === $data['original_id'] && $lesson->type === 'lessons') {
                $this->io->verbose(sprintf('Reusing already imported lesson "%s"', $uname));

                return $lesson;
            }

            $this->io->warning(sprintf('Lesson "%s" already exists, skipping import', $uname));

            return false;
        }

        $lesson = $this->Lessons->newEntity(compact('uname'));
        $lesson->created_by = UsersTable::ADMIN_USER;
        $lesson->modified_by = UsersTable::ADMIN_USER;
        $lesson->created = $data['created'] ?? FrozenTime::now(); // @phpstan-ignore-line
        $lesson->modified = $data['modified'] ?? FrozenTime::now(); // @phpstan-ignore-line
        if (!empty($data['extra']) && is_string($data['extra'])) {
            $data['extra'] = json_decode($data['extra'], true);
        }

        $lesson = $this->Lessons->patchEntity($lesson, array_merge_recursive($data, [
            'extra' => [
                'imported' => [
                    'id' => $data['original_id'],
                    'uname' => $data['original_uname'],
        ...
        /** @var \BEdita\Core\Model\Entity\ObjectEntity $lesson */
        $lesson = $this->Lessons->saveOrFail($lesson);
     ...
     * @param AuthorData $data Data of the author to be imported
    ...
    protected function findOrCreateAuthor(string $uname, array $data): Profile|false
    {
        ...
        $author = $this->Profiles->newEntity(compact('uname'));
        $author->created_by = UsersTable::ADMIN_USER;
        $author->modified_by = UsersTable::ADMIN_USER;
        $author->created = $data['created'] ?? FrozenTime::now(); // @phpstan-ignore-line
        $author->modified = $data['modified'] ?? FrozenTime::now(); // @phpstan-ignore-line
        if (!empty($data['extra']) && is_string($data['extra'])) {
            $data['extra'] = json_decode($data['extra'], true);
        }

        $author = $this->Profiles->patchEntity($author, array_merge_recursive($data, [
            'extra' => [
                'imported' => [
     ...
     * @param array $data Data of the link to be imported
    ...
    protected function findOrCreateLink(string $uname, array $data): Link|false
    {
        ...
        $link = $this->Links->newEntity(compact('uname'));
        $link->created_by = UsersTable::ADMIN_USER;
        $link->modified_by = UsersTable::ADMIN_USER;
        $link->created = $data['created'] ?? FrozenTime::now(); // @phpstan-ignore-line
        $link->modified = $data['modified'] ?? FrozenTime::now(); // @phpstan-ignore-line
        if (!empty($data['extra']) && is_string($data['extra'])) {
            $data['extra'] = json_decode($data['extra'], true);
        }

        $link = $this->Links->patchEntity($link, array_merge_recursive($data, [
            'extra' => [
                'imported' => [
                    'id' => $data['original_id'],
                    'uname' => $data['original_uname'],
     ...
     * @param MediaData $data Data of the media to be imported
    ...
    protected function findOrCreateMedia(string $uname, array $data, string $mediaType, bool $download = false): Media|false
    {
        ...
        $media = $Media->newEntity(compact('uname'));
        $media->created_by = UsersTable::ADMIN_USER;
        $media->modified_by = UsersTable::ADMIN_USER;
        $media->created = $data['created'] ?? FrozenTime::now(); // @phpstan-ignore-line
        $media->modified = $data['modified'] ?? FrozenTime::now(); // @phpstan-ignore-line
        if (!empty($data['extra']) && is_string($data['extra'])) {
            $data['extra'] = json_decode($data['extra'], true);
        }
        ...
        /** @var \BEdita\Core\Model\Entity\Media $media */
        $media = $Media->patchEntity($media, array_merge_recursive($data, [
            'extra' => [
                'imported' => [
                    'id' => $data['original_id'],
                    'uname' => $data['original_uname'],
    ...
    protected function getBe3FolderContents(int $id, string $order = 'asc', array $excludedTypes = ['b_e_file', 'image', 'application', 'audio', 'video', 'caption']): array
    {
        $contents = $this->sourceConnection->newQuery()
            ->select([
                ...
                'title' => 'o.title',
                'description' => 'o.description',
                'body' => new FunctionExpression('COALESCE', ['l.body' => 'identifier', 'c.body' => 'identifier']),
                'status' => 'o.status',
